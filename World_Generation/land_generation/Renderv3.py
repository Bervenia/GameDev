...
class Render():
    ...
    #def load_tiles(self, level_data, batch):
    ...
    def auto_tile_region(self, level, x, y,batch, auto_tile = False,):
        sprites = []
        width = len(level)
        height = len(level)
        ...
        for i in range(y,height):
            for j in range(x,width):
                ...
                if transition[0] == 0:
                    ...
                    sprite = pyglet.sprite.Sprite(img = tile,x= x_pos, y = y_pos, batch = batch)
                    ...
                    sprites.append(pyglet.sprite.Sprite(img = tile,x= x_pos, y = y_pos, batch = batch))
                elif transition[0] > 1:                    
